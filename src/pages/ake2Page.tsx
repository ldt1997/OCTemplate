      ...
      <div className="relative flex h-full">
        ...
        <section className="relative min-w-0 flex-1" aria-label="海报预览">
          <Ake2Viewport>
            {(displayWidth) => (
              <>
                {resourceError || fontError ? (
                  <p role="alert" className="p-6 text-sm text-destructive">{resourceError || fontError}</p>
                ) : resources && resourcesReady && fontsReady ? (
                  <Ake2Canvas
                    displayWidth={displayWidth} form={form} resources={resources}
                    onError={setResourceError} onImagePositionChange={onImagePositionChange}
                  />
                ) : (
                  <div role="status" className="flex aspect-[4/3] items-center justify-center text-sm text-muted-foreground">
                    {fontsReady ? "正在加载海报图片…" : "正在加载模板字体…"}
                  </div>
                )}
                {exportError && <p role="alert" className="px-4 py-3 text-sm text-destructive">{exportError}</p>}
              </>
            )}
          </Ake2Viewport>
